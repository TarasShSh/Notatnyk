...
void Widget::on_filteredButton_clicked()
{
    ...
    {
        searchNhide(2, groups[0], groups[1], groups[2], groups[3], groups[4]);
    }
    if(!(ui->noneCheckBox->isChecked()))
    {
        if(!(ui->selfCheckBox->isChecked()))
        {
            searchNhide(2, groups[0], groups[0], groups[2], groups[3], groups[4]);
        }
        searchNhide(2, groups[1], groups[1], groups[2], groups[3], groups[4]);
    }

    if(!(ui->workCheckBox->isChecked()))
    {
        searchNhide(2, groups[0], groups[1], groups[1], groups[3], groups[4]);
    }
    if(!(ui->studyCheckBox->isChecked()))
    {
        searchNhide(2, groups[0], groups[1], groups[2], groups[2], groups[4]);
    }
    if(!(ui->todoCheckBox->isChecked()))
    {
        searchNhide(2, groups[0], groups[1], groups[2], groups[3], groups[3]);
    }
...
{
    QRegularExpression searchExp(arg1, QRegularExpression::CaseInsensitiveOption);
    QRegularExpression regExp[1]={searchExp};
    ui->foundedNumber->setText(QString("%1").arg(searchNhide(0, arg1, arg1, arg1, arg1, arg1)));
}

int Widget::searchNhide(int column, QString regExp, QString regExp1, QString regExp2, QString regExp3, QString regExp4)
{
    int times = 0;
    ...
    for (int child = 0; child < (ui->notesTree->topLevelItem(father)->childCount());)
        {
                qDebug() << "Child count = " << child;
                        ...
                        || ui->notesTree->topLevelItem(father)->child(child)->text(column).contains(regExp1)
                        || ui->notesTree->topLevelItem(father)->child(child)->text(column).contains(regExp2)
                        || ui->notesTree->topLevelItem(father)->child(child)->text(column).contains(regExp3)
                        || ui->notesTree->topLevelItem(father)->child(child)->text(column).contains(regExp4)
                           )
                {
                    qDebug() << "contain " << true;
